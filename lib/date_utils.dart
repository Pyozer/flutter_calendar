...
class Utils {
  ...
  static bool _isMondayFirstDayOfWeek = true;
  ...
  static DateTime lastDayOfMonth(DateTime month) {
    final beginningNextMonth = (month.month < 12)
        ? DateTime(month.year, month.month + 1, 1, 12)
        : DateTime(month.year + 1, 1, 1, 12);
  ...
  static Iterable<DateTime> daysInRange(DateTime start, DateTime end) sync* {
    ...
    var prevDay = start;
    while(!isSameDay(prevDay, end)) {
      yield prevDay;
      prevDay = prevDay.add(Duration(days: 1));
    }
    yield end;
  }
